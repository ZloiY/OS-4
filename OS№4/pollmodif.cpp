...
DWORD WINAPI ConnectThread(LPVOID pParam)
{
    ...
    __try
    {
        if (INVALID_SOCKET == (csock = socket(AF_INET6, SOCK_STREAM, 0)))
        {
            ERR("socket");
            __leave;
        }
        ...
        if (SOCKET_ERROR == connect(csock, (SOCKADDR*)&addrLoopback, sizeof (addrLoopback)))
        {
        ...
        if (ret)
        {
            if (fdarray.revents & POLLWRNORM)
            {
                ...
                //Send data
                if (SOCKET_ERROR == (ret = send(csock, TST_MSG, sizeof (TST_MSG), 0)))
                {
                    ERR("send");
                    __leave;
                }
